...
import torch
import os
from dataloader import Dataloader,OPTDataloader
import random
random.seed(3407)
...
# opt_data_path = f"/ssd/liwentao/ocp_main/new_calculate/generate_opt_2/all_gen_opt_{split}.h5"
# neb_data_path = f"/ssd/liwentao/ocp_main/new_calculate/all_gen_neb/all_gen_neb_{split}.h5"
opt_data_path = f"/ssd/liwentao/MLFF-TS-Search/final_data/mix_data/all_gen_opt_{split}.h5"
neb_data_path = f"/ssd/liwentao/MLFF-TS-Search/final_data/mix_data/new_{split}_fold1.h5"
dataloaders = {
    # "train": Dataloader(data_path, "train"),
    # "test": Dataloader(data_path, "test"),
    'neb': Dataloader(neb_data_path),
    'opt': OPTDataloader(opt_data_path),
...
os.makedirs(f"../final_data/mix_data/{split}", exist_ok=True)
# os.system(f"rm -rf transition1x_s2ef/{split}/*")

db = lmdb.open(
    # f"catalyst_select_refine_shift/{split}/Rh.lmdb",
    f"../final_data/mix_data/{split}/Rh.lmdb",
...
dataset = LmdbDataset({"src": f"../final_data/mix_data/{split}"})
print(dataset[0].y)
print(dataset[1].y)
# print(dataset[229].y)
# print(dataset[230].y)