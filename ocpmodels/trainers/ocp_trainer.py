...
class OCPTrainer(BaseTrainer):
    ...
    def train(self, disable_eval_tqdm: bool = False) -> None:
        ...
        for epoch_int in range(
            ...
            # 这里只训练部分的数据集看看效果
            train_iter_num = len(self.train_loader)

            for i in range(skip_steps, train_iter_num):
                self.epoch = epoch_int + (i + 1) /train_iter_num
                self.step = epoch_int * train_iter_num + i + 1
                self.model.train()
        ...
        # self.model = torch.load(os.path.join("./checkpoints/2024-03-24-18-05-52", "best_checkpoint.pt")) # self.config["cmd"]["checkpoint_dir"]
        self.load_checkpoint(os.path.join(self.config["cmd"]["checkpoint_dir"], "best_checkpoint.pt"))
