...
if __name__ == "__main__":
    name = "Cat59"
    xyz_file = f"error/{name}/{name}_reactant.xyz"
    atoms_r = read(xyz_file, ":")[0]

    xyz_file = f"error/{name}/{name}_product.xyz"
    atoms_p = read(xyz_file, ":")[0]

    images = interpolate_custom(atoms_r, atoms_p, num_intermediate=10)
    from ase.io import write

    write(f"{name}_ours.traj", images)
